...
- **`MEMORI_ENTITY_ID is required`** — namespace not configured. Add `MEMORI_ENTITY_ID` to the `env` block of `.claude/settings.local.json` with any stable string (e.g. slugified `git config user.email`, hostname, or a UUID). On first failure Claude Code is instructed by `SKILL.md` to do this automatically when it can infer a value.
